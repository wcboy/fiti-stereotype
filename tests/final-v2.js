...
// 最终优化的 Pattern：确保每个 pattern 都能被随机结果匹配到
// 维度顺序: FOCUS, MEMORY, RISK, DECISION, STRATEGY, EXECUTE, LEARN, EMOTION, SOCIAL, BOUNDARY, CONFLICT, PATIENCE
// 直接从 types.json 读取，不再硬编码
const finalPatterns = {};
for (const t of standardTypes) {
  finalPatterns[t.code] = t.pattern;
}

function createFinalTypes() {
  return standardTypes.map(t => ({
    ...t,
    pattern: finalPatterns[t.code] || t.pattern,
  }));
}
...
const finalTypes = createFinalTypes();
const { results, levelDist } = runSimulation(finalTypes);
...
console.log('\n📝 最终 Pattern JSON:');
console.log(JSON.stringify(finalPatterns, null, 2));
